...
| **多主题色** | Day / Night / Pink / Blue / Purple / Green |
| **隐私模式** | 一键模糊持仓金额 |
...
- 服务启动后会自动启动 APScheduler。
- 交易日交易时段按固定分钟间隔后台更新：行情、基金估值、基金涨跌榜、用户持仓。
- 页面默认优先展示缓存/数据库中的最近可用数据；后台刷新完成后自动呈现新数据。
- 因此无需“进入某个页面”才触发更新，页面主要承担展示与手动刷新入口。
- 收盘后系统会轮询基金公司发布的官方净值；一旦发布即覆盖临时估值并同步到前端状态栏“官方净值已更新 x/y”。
